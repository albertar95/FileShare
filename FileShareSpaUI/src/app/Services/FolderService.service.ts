...
export class FolderService {
  ...
  add(folder: CreateFolderDTO): Observable<boolean> {
    folder = new CreateFolderDTO('tret', true, true, false, 'pth', '5da96548-7b4b-460d-b8ad-b2a4f666f13c');
  ...
  update(folder: UpdateFolderDTO): Observable<boolean> {
    folder = new UpdateFolderDTO('dafe4f4b-d74f-4e72-828f-0ed28c107b90', 'rename from angular', true, false);
  ...
  get(folderId: string): Promise<Folder> {
    folderId = 'dafe4f4b-d74f-4e72-828f-0ed28c107b90';//for test
    return this.http.get<Folder>(this.baseApiUrl + '/Folder/GetFolderById/' + folderId, this.httpOptions).toPromise()
      .then(response => response as Folder);
  }
}
